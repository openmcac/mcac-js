...
test('it returns the following Sunday at 9:30 Montreal Time', function(assert) {
  var result = nextService(new Date("2012-12-18T03:51:57-05:00"));
  assert.equal(result.toDate().getTime(),
               new Date("2012-12-23T09:30:00-05:00").getTime());
});


test('it returns the following Sunday at 9:30 Montreal Time (DST)', function(assert) {
  var result = nextService(new Date("2012-10-14T09:32:00-04:00"));
  assert.equal(result.toDate().getTime(),
               new Date("2012-10-21T09:30:00-04:00").getTime());
});

test('it returns the following Sunday when currently Sunday', function(assert) {
  var result = nextService(new Date("2012-12-23T09:32:00-05:00"));
  assert.equal(result.toDate().getTime(),
               new Date("2012-12-30T09:30:00-05:00").getTime());
});

test('it returns the current Sunday when Sunday before 9:30am', function(assert) {
